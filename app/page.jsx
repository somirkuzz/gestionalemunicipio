...
export default function Page() {
  ...
  // Controllo se l'utente appartiene all'ufficio anagrafe
  const hasAnagrafeAccess = () => ["CAMBIO_NOME", "ADOZIONE", "DISCONOSCIMENTO", "DIVORZIO", "UNIONE_CIVILE"].some(p => can(p));
    ...
    <div style={pageBg}>
      <nav style={navStyle}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
          <div style={{textAlign:'right'}}><b style={{fontSize:'14px'}}>{user.n}</b><br/><small style={{fontSize:'10px'}}>{user.r}</small></div>
      ...
      <div style={container}>
        {pagina === 'home' && (
          <div style={gridStyle}>
            {/* Solo chi ha ruoli anagrafe vede questo tasto */}
            {hasAnagrafeAccess() && <Card t="Ufficio Anagrafe" d="Moduli Civili" c="#10b981" onClick={()=>setPagina('menu_anagrafe')} />}
        ...
        {pagina === 'menu_archivio' && (
          <div>
            <button onClick={()=>setPagina('home')} style={backBtn}>← Indietro</button>
            <div style={gridStyle}>
              {/* L'area amministrativa NON vede questi registri nell'archivio */}
              {hasAnagrafeAccess() && (
                <>
                  <Card t="Registro Nomi" d="Cambi nomi" c="#ef4444" onClick={()=>fetchArchivio('anagrafe_nomi')} />
                  <Card t="Registro Adozioni" d="Adozioni" c="#ef4444" onClick={()=>fetchArchivio('anagrafe_adozioni')} />
                  <Card t="Registro Disc." d="Disconoscimenti" c="#ef4444" onClick={()=>fetchArchivio('anagrafe_disconoscimento')} />
                  <Card t="Registro Divorzi" d="Divorzi" c="#ef4444" onClick={()=>fetchArchivio('anagrafe_divorzi')} />
                  <Card t="Registro Unioni" d="Unioni" c="#ef4444" onClick={()=>fetchArchivio('anagrafe_unioni')} />
                </>
              )}
              {/* Il registro congedi è l'unico visibile agli amministrativi nell'archivio */}
        ...
        {pagina.startsWith('form_') && (
          <div style={{maxWidth:'500px', margin:'0 auto'}}>
            ...
            <div style={formCard}>
              ...
              <label style={labStyle}>Data</label><input style={inputStyle} value={form.data || ''} onChange={(e)=>setForm({...form, data:e.target.value})} />
              <label style={labStyle}>Operatore</label><input style={inputStyle} value={form.nome_dipendente || ''} onChange={(e)=>setForm({...form, nome_dipendente:e.target.value})} />
              ...
              {(pagina==='form_adozione' || pagina==='form_disconoscimento') && (<>
                <label style={labStyle}>Figlio/a</label><input style={inputStyle} onChange={(e)=>setForm({...form, nome_figlio:e.target.value})} />
                <label style={labStyle}>Padre</label><input style={inputStyle} onChange={(e)=>setForm({...form, nome_padre:e.target.value})} />
                <label style={labStyle}>Madre</label><input style={inputStyle} onChange={(e)=>setForm({...form, nome_madre:e.target.value})} />
              </>)}
              {(pagina==='form_divorzio' || pagina==='form_unione') && (<>
                <label style={labStyle}>Coniuge 1</label><input style={inputStyle} onChange={(e)=>setForm({...form, nome_coniuge1:e.target.value})} />
                <label style={labStyle}>Coniuge 2</label><input style={inputStyle} onChange={(e)=>setForm({...form, nome_coniuge2:e.target.value})} />
              </>)}
              {pagina==='form_congedo' && (<>
              ...
              <button style={submitBtn} onClick={() => {
                const map = { 
                    form_nome: 'anagrafe_nomi', 
                    form_adozione: 'anagrafe_adozioni', 
                    form_disconoscimento: 'anagrafe_disconoscimento', 
                    form_divorzio: 'anagrafe_divorzi', 
                    form_unione: 'anagrafe_unioni', 
                    form_congedo: 'congedi' 
                };
        ...
        {pagina === 'visualizza_archivio' && (
          <div style={formCard}>
            ...
            <h2 style={{color:'#1e3a8a'}}>{tabellaCorrente.toUpperCase().replace('ANAGRAFE_','')}</h2>
