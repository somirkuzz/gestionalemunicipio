...
export default function Page() {
  ...
  const inviaPratica = async (tabella) => {
    try {
      ...
      if (res.ok) { setPagina('successo'); setForm({}); }
        ...
        alert("Errore Database: " + (err.message || "Verifica i nomi delle colonne")); 
      }
  ...
  if (!user) return (
    <div style={loginBg}><div style={loginCard}>
      <h2 style={{color:'#1e3a8a', marginBottom:'10px'}}>MUNICIPIO ATLANTIS</h2>
      <p style={{fontSize:'12px', color:'#64748b', marginBottom:'20px'}}>Sistema Gestionale Interno</p>
      <input style={inputStyle} placeholder="Inserisci il tuo Nickname" value={nick} onChange={(e)=>setNick(e.target.value)} />
      <button style={submitBtn} onClick={()=>{ if(DIPENDENTI[nick]) setUser({n:nick, r:DIPENDENTI[nick]}); else alert("Nickname non riconosciuto."); }}>ENTRA</button>
    </div></div>
  );
  ...
  return (
    <div style={pageBg}>
      <nav style={navStyle}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
          <div style={{textAlign:'right'}}>
            <b style={{fontSize:'14px', display:'block'}}>{user.n}</b>
            <small style={{fontSize:'10px', textTransform:'uppercase'}}>{user.r.replace(/_/g, ' ')}</small>
          </div>
      ...
      <div style={container}>
        {pagina === 'home' && (
          <div style={gridStyle}>
            <Card t="Ufficio Anagrafe" d="Gestione moduli civili" c="#10b981" onClick={()=>setPagina('menu_anagrafe')} />
            {can("CONGEDO") && <Card t="Modulo Congedi" d="Richiesta ferie e assenze" c="#1e3a8a" onClick={()=>setPagina('form_congedo')} />}
            {can("ARCHIVIO") && <Card t="Archivio Centrale" d="Consultazione registri" c="#ef4444" onClick={()=>setPagina('menu_archivio')} />}
          </div>
        )}
        ...
        {pagina.startsWith('form_') && (
          <div style={{maxWidth:'500px', margin:'0 auto'}}>
            ...
            <div style={formCard}>
              <h2 style={{marginTop:0, color:'#1e3a8a', textTransform:'uppercase'}}>{pagina.replace('form_','').replace('_',' ')}</h2>
              
              <label style={labStyle}>Data Operazione</label>
              <input style={inputStyle} value={form.data || ''} onChange={(e)=>setForm({...form, data:e.target.value})} />
              
              <label style={labStyle}>Operatore Responsabile</label>
              ...
              {pagina==='form_congedo' && (<>
                <label style={labStyle}>Periodo Assenza</label><input style={inputStyle} placeholder="es. dal 01/04 al 05/04" onChange={(e)=>setForm({...form, periodo:e.target.value})} />
              ...
              <button style={submitBtn} onClick={() => {
                let tab = '';
                if(pagina === 'form_nome') tab = 'anagrafe_nomi';
                else if(pagina === 'form_adozione') tab = 'anagrafe_adozioni';
                else if(pagina === 'form_disconoscimento') tab = 'anagrafe_disconoscimento';
                else if(pagina === 'form_divorzio') tab = 'anagrafe_divorzi';
                else if(pagina === 'form_unione') tab = 'anagrafe_unioni';
                else if(pagina === 'form_congedo') tab = 'congedi'; // Corretto qui
                inviaPratica(tab);
        ...
        {pagina === 'successo' && <div style={{textAlign:'center', padding:'50px'}}><div style={formCard}><h2>✅ Operazione Completata</h2><p>I dati sono stati salvati correttamente nell'archivio.</p><button onClick={()=>setPagina('home')} style={submitBtn}>TORNA ALLA HOME</button></div></div>}
      </div>
    </div>
  );
}
